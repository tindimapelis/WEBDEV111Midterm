const eventCatalog = [
  {
    ...
    tiers: [
      { id: "general", name: "General", price: "PHP 2,190", perks: "Main floor access + welcome tote" },
      { id: "plus", name: "Plus", price: "PHP 4,390", perks: "Fast lane entry + lounge seating" },
      { id: "vip", name: "VIP", price: "PHP 7,190", perks: "Front pit + artist zine pack" }
    ]
  },
  {
    id: "pixelplay",
    ...
    tiers: [
      { id: "day", name: "Day Pass", price: "PHP 1,390", perks: "All exhibits until 6PM" },
      { id: "creator", name: "Creator", price: "PHP 3,290", perks: "Workshops + networking mix" },
      { id: "allin", name: "All In", price: "PHP 5,190", perks: "Full-day access + merch bundle" }
    ]
  },
  {
    id: "citylights",
    ...
    tiers: [
      { id: "rooftop", name: "Rooftop", price: "PHP 2,590", perks: "Concert access + drink token" },
      { id: "premium", name: "Premium", price: "PHP 4,890", perks: "Reserved deck rail section" },
      { id: "backstage", name: "Backstage", price: "PHP 7,790", perks: "Soundcheck + photo wall pass" }
    ]
  }
];
...
const userSessionKey = "nexusUserAuthenticated";
...
function initActiveNav() {
  ...
  document.querySelectorAll(".nav-link, .admin-entry, .user-entry").forEach((link) => {
    const targetPath = link.getAttribute("href");
    const activeClass = link.classList.contains("admin-entry") || link.classList.contains("user-entry")
      ? "is-current"
      : "is-active";
...
function isUserAuthenticated() {
  return window.sessionStorage.getItem(userSessionKey) === "true";
}
...
function initUserLogin() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const password = String(formData.get("user_password") || "").trim();
    ...
    if (status) {
      status.textContent = "Login successful. Redirecting to tickets...";
      status.classList.remove("is-error");
      status.classList.add("is-success");
    }
