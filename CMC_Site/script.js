...
const adminAccountMenus = document.querySelectorAll(".admin-account-menu");

adminAccountMenus.forEach((menu) => {
  const trigger = menu.querySelector(".admin-account-trigger");
  ...
  trigger?.addEventListener("click", (event) => {
    ...
    adminAccountMenus.forEach((otherMenu) => {
      if (otherMenu !== menu) {
        otherMenu.classList.remove("is-open");
        otherMenu.querySelector(".admin-account-trigger")?.setAttribute("aria-expanded", "false");
      }
    });
  ...
  menu.querySelectorAll(".admin-account-link").forEach((link) => {
...
if (adminSidebarLinks.length > 0 && currentPage) {
  ...
  const adminNavGroups = new Map([
    ...
    [
      "superadmin/management.html",
      new Set([
        "superadmin/management.html",
        "superadmin/full-access.html",
        "superadmin/registration.html",
        "superadmin/minigames.html",
        "superadmin/security.html",
        "superadmin/strategy.html",
      ]),
    ],
  ]);

  const mappedCurrentPage =
    Array.from(adminNavGroups.entries()).find(([, pages]) => pages.has(currentPage))?.[0] || currentPage;
  ...
  adminSidebarLinks.forEach((link) => {
    ...
    const isCurrent = normalizedHref === mappedCurrentPage;
...
const safeStorage = {
  ...
  write(key, value) {
    try {
      ...
      // Ignore storage failures so the UI still works offline/private mode.
    }
  },
};

const MERCH_ACTIVITY_KEY = "cmc_merch_activity";
const MERCH_EMAIL_KEY = "cmc_merch_emails";

const persistMerchActivity = (entry) => {
  const nextEntries = [entry, ...safeStorage.read(MERCH_ACTIVITY_KEY)].slice(0, 30);
  safeStorage.write(MERCH_ACTIVITY_KEY, nextEntries);
};

const persistMerchEmail = (entry) => {
  const nextEntries = [entry, ...safeStorage.read(MERCH_EMAIL_KEY)].slice(0, 30);
  safeStorage.write(MERCH_EMAIL_KEY, nextEntries);
};

if (artCarousel) {
  const artImage = artCarousel.querySelector("[data-art-carousel-image]");
  const artStage = artCarousel.querySelector(".art-carousel-stage");
  const artTitle = artCarousel.querySelector("[data-art-carousel-title]");
  const artCaption = artCarousel.querySelector("[data-art-carousel-caption]");
  ...
  const artSlides = [
    {
      image: "monthlyart_1.jpg",
      title: "-Artwork Title-",
      caption: "Created by -Placeholder Artist-",
    },
    {
      image: "monthlyart_2.jpg",
      title: "-Artwork Title-",
      caption: "Created by -Placeholder Artist-",
    },
    {
      image: "monthlyart_3.jpg",
      title: "-Artwork Title-",
      caption: "Created by -Placeholder Artist-",
    },
  ];
  ...
  const renderArtSlide = () => {
    ...
    const slide = artSlides[activeArtIndex];
    artStage?.classList.add("is-transitioning");
    artImage.src = slide.image;
    artImage.alt = slide.title;

    if (artTitle) {
      artTitle.textContent = slide.title;
    }

    if (artCaption) {
      artCaption.textContent = slide.caption;
    }

    window.setTimeout(() => {
      artStage?.classList.remove("is-transitioning");
    }, 220);
  };
...
if (eventsFilter) {
  ...
  const modalFiles = eventsModal?.querySelector("[data-events-modal-files]");
  ...
  const filenameToLabel = (filepath) => {
  ...
  const openEventsModal = (card, trigger) => {
    ...
    const derivedTitle = filenameToLabel(activeEventImages[0] || "");
    const explicitDescription = card.getAttribute("data-event-description") || "";
    const explicitMeta = card.getAttribute("data-event-meta") || "";
    const readableTitle = derivedTitle || card.getAttribute("data-event-title") || "CMC Event";
    const hasReadableDescription = explicitDescription && !/[Ãâð�]/.test(explicitDescription);
    const hasReadableMeta = explicitMeta && !/[Ãâð�]/.test(explicitMeta);

    modalTitle.textContent = readableTitle;
    modalStatus.textContent = card.getAttribute("data-event-status") || "Event";
    modalDescription.textContent =
      explicitDescription && !/[ð�]/.test(explicitDescription)
        ? explicitDescription
        : `${readableTitle} is part of the CMC event archive. Open the carousel to browse the related poster and gallery images for this event set.`;
    modalMeta.textContent =
      explicitMeta && !/[ð�]/.test(explicitMeta)
        ? explicitMeta
        : `Photo set: ${activeEventImages.map(filenameToLabel).filter(Boolean).join(" • ")}`;

    if (!hasReadableDescription) {
      modalDescription.textContent = `${readableTitle} is part of the CMC event archive. Open the carousel to browse the related poster and gallery images for this event set.`;
    }

    if (!hasReadableMeta) {
      modalMeta.textContent = `Photo set: ${activeEventImages.map(filenameToLabel).filter(Boolean).join(" | ")}`;
    }

    if (modalFiles) {
      modalFiles.innerHTML = activeEventImages
        .map((imagePath) => `<span>${filenameToLabel(imagePath)}</span>`)
        .join("");
    }

    renderEventModalImage();
    eventsModal.hidden = false;
    eventsModal.scrollIntoView({ behavior: "smooth", block: "start" });
  };
...
const merchProductPanel = document.querySelector("[data-product-panel]");

if (merchProductPanel) {
  const productCards = document.querySelectorAll(".merch-catalog-card");
  const merchToast = document.querySelector("[data-merch-toast]");
  const productImage = merchProductPanel.querySelector("[data-product-modal-image]");
  const productTitle = merchProductPanel.querySelector("[data-product-modal-title]");
  const productDescription = merchProductPanel.querySelector("[data-product-modal-description]");
  const productPrice = merchProductPanel.querySelector("[data-product-modal-price]");
  const productColor = merchProductPanel.querySelector("[data-product-modal-color]");
  const productColorGroup = productColor?.closest(".merch-product-option-group");
  const productColors = merchProductPanel.querySelector("[data-product-modal-colors]");
  const productOptionLabel = merchProductPanel.querySelector("[data-product-modal-option-label]");
  const productSizes = merchProductPanel.querySelector("[data-product-modal-sizes]");
  const productQty = merchProductPanel.querySelector("[data-product-modal-qty]");
  const productAdd = merchProductPanel.querySelector("[data-product-modal-add]");
  const productClose = merchProductPanel.querySelector("[data-product-panel-close]");
  let currentProduct = null;
  let currentQty = 1;
  let lastProductTrigger = null;
  let selectedOption = "";
  let selectedColor = "";
  let merchToastTimer = null;
  ...
  const openProduct = (card) => {
    ...
    if (productColor) {
      productColor.textContent = currentProduct.color === "None" ? "Default" : currentProduct.color;
    }
    ...
    if (productSizes) {
      productSizes.innerHTML = currentProduct.sizes
        .map((size, index) => {
          ...
          const variationImage = currentProduct.variationImages?.[trimmedSize];
          const isVariationPicker = currentProduct.optionLabel === "Variation" && variationImage;

          return isVariationPicker
            ? `<button class="merch-size-chip merch-variation-chip${index === 0 ? " is-selected" : ""}" type="button" data-option-value="${trimmedSize}"><img src="${variationImage}" alt="${trimmedSize} lanyard preview" /><span>${trimmedSize}</span></button>`
            : `<button class="merch-size-chip${index === 0 ? " is-selected" : ""}" type="button" data-option-value="${trimmedSize}">${trimmedSize}</button>`;
        })
        .join("");
    ...
    merchProductPanel.hidden = false;
    merchProductPanel.scrollIntoView({ behavior: "smooth", block: "start" });
    productClose?.focus();
  };

  const closeProduct = () => {
    merchProductPanel.hidden = true;
  ...
  merchProductPanel.querySelectorAll("[data-qty-change]").forEach((button) => {
    button.addEventListener("click", () => {
  ...
  productAdd?.addEventListener("click", () => {
    ...
    for (let count = 0; count < currentQty; count += 1) {
      const itemNameParts = [currentProduct.name];

      if (selectedColor && selectedColor !== "None" && selectedColor !== "Coming Soon") {
        itemNameParts.push(selectedColor);
      }

      if (selectedOption && selectedOption !== "Work in Progress") {
        itemNameParts.push(selectedOption);
      }

      const addEvent = new CustomEvent("merch:add-to-cart", {
        detail: {
          ...currentProduct,
          cartLabel: itemNameParts.join(" • "),
        },
      });
    ...
    if (merchToast) {
      window.clearTimeout(merchToastTimer);
      merchToast.hidden = false;
      merchToast.style.animation = "none";
      void merchToast.offsetWidth;
      merchToast.style.animation = "";
      merchToastTimer = window.setTimeout(() => {
        merchToast.hidden = true;
      }, 2600);
    }
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !merchProductPanel.hidden) {
      closeProduct();
    }
  });
}
...
if (merchCartModal) {
  ...
  const cartBuyNow = merchCartModal.querySelector("[data-cart-buy]");
  ...
  const renderCart = () => {
    ...
    if (count === 0) {
      cartList.innerHTML = '<li class="merch-cart-empty">No items yet. Add a product to begin.</li>';
      return;
    }
  ...
  document.addEventListener("merch:add-to-cart", (event) => {
    ...
    cartItems.push({
      name: item.cartLabel || item.name || "CMC Item",
      price: Number(item.price || 0),
    });
    persistMerchActivity({
      item: item.cartLabel || item.name || "CMC Item",
      price: Number(item.price || 0),
      source: "Add to cart",
  ...
  cartBuyNow?.addEventListener("click", () => {
    ...
    const email = window.prompt("Enter your email so we can send delivery schedule updates:");
    ...
    window.alert(`Thanks! Delivery schedule updates will be sent to ${trimmedEmail}.`);
    closeCart();
  });

  merchCartModal.addEventListener("click", (event) => {
    if (event.target === merchCartModal || event.target.closest("[data-cart-close]")) {
      closeCart();
    }
  });
...
const merchManagementPanels = document.querySelectorAll("[data-merch-management]");

if (merchManagementPanels.length > 0) {
  const activityEntries = safeStorage.read(MERCH_ACTIVITY_KEY);
  const emailEntries = safeStorage.read(MERCH_EMAIL_KEY);
  ...
  const totalRevenue = activityEntries.reduce((sum, entry) => sum + Number(entry.price || 0), 0);

  merchManagementPanels.forEach((panel) => {
    const orderCount = panel.querySelector("[data-merch-order-count]");
    const emailCount = panel.querySelector("[data-merch-email-count]");
    const revenueTotal = panel.querySelector("[data-merch-revenue-total]");
    const activityList = panel.querySelector("[data-merch-activity-list]");
    const emailList = panel.querySelector("[data-merch-email-list]");

    if (orderCount) {
      orderCount.textContent = String(activityEntries.length);
    }

    if (emailCount) {
      emailCount.textContent = String(emailEntries.length);
    }

    if (revenueTotal) {
      revenueTotal.textContent = new Intl.NumberFormat("en-PH", {
        style: "currency",
        currency: "PHP",
        maximumFractionDigits: 0,
      }).format(totalRevenue);
        ...
        activityEntries.length > 0
          ? activityEntries
              ...
              .map(
                (entry) => `
                  <div>
                    <strong>${entry.item || "CMC Item"}</strong>
                    <span>${formatTimestamp(entry.createdAt)} | ${new Intl.NumberFormat("en-PH", {
                      style: "currency",
                      currency: "PHP",
                      maximumFractionDigits: 0,
                    }).format(Number(entry.price || 0))}</span>
                  </div>
                `
              )
              .join("")
          : '<div><strong>No cart activity yet</strong><span>Items added from the merch page will appear here.</span></div>';
    }
              ...
              .map(
                (entry) => `
                  <div>
                    ...
                    <span>${formatTimestamp(entry.createdAt)} | ${entry.items?.length || 0} items</span>
                  </div>
                `
              )
              .join("")
          : '<div><strong>No delivery email signups yet</strong><span>Buy Now submissions will appear here.</span></div>';
    }
  });
}
